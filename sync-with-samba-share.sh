...
function main() {
    ...
    *)
    ...
    rsync --recursive --times --partial --out-format="%n" --delete-during --backup --exclude=".recycle/*" --backup-dir=".recycle/$(date +%Y%m%d-%H%M%S)/" "${arguments[@]}" "$rsync_source" "$rsync_target" 2>/dev/null || exit 1
}
